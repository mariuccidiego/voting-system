...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['id'])) {
        if (isset($_POST['username']) && isset($_POST['password'])) {
            ...
            $sql = "SELECT * FROM votante WHERE username = '$username'";
            ...
            if ($query->num_rows < 1) {
                ...
                if ($_POST['id'] == $row['votazione_id']) {
                    if ($password == $row['pwd']) {
                        $_SESSION['votante'] = $row['username'];
                    } else {
                        $_SESSION['error'] = 'Username o Password sbagliati';
                    }
                } else {
                    $_SESSION['error'] = 'Ricontrolla che il link sia corretto';
                }
